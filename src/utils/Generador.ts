  ...
  for (let i = 0; i < eventos.length - 1; i++) {
    for (let j = i + 1; j < eventos.length; j++) {
      if (
        ...
        // Siempre hay conflicto si hay solapamiento
        penalizacion++;
  ...
  for (const materia of materias) {
    ...
    while (sesionesAsignadas < materia.sesiones) {
      ...
      const conflicto = existeConflictoHorario(
        ...
        undefined,
        materia._id // Pasamos el id de la materia
      );
...
// Función modificada para verificar conflictos de horario incluyendo la misma materia el mismo día
function existeConflictoHorario(
  ...
  return eventos.some((evento) => {
    ...
    const conflictoMismaMateria =
      materiaId && evento.materia
        ? evento.materia._id.equals(materiaId) && eventoHorario.dia === horarioC.dia
        : false;

    // Eliminamos el "|| true" para que el conflicto dependa realmente de las condiciones.
    return solapamiento && (conflictoProfesor || conflictoSalon || conflictoMismaMateria);
  });
}
